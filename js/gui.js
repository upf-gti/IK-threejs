                        ...
                        if(constraint) {
                            ...
                            for(let c in constraint) {
                                ...
                                widgets.addDefault(c, constraint[c], v=>{
                                    ...
                                    this.editor.FABRIKSolver.setConstraintToBone( this.editor.fabrikChains[i].name, j, constraint ); });
                            }
                            widgets.addButton(null, "Remove constraint", { callback: v => {
                                this.editor.fabrikChains[i].constraints[j] = null;
                                this.editor.FABRIKSolver.setConstraintToBone( this.editor.fabrikChains[i].name, j, null );
                                widgets.refresh();
                            }})
                        }else{
    ...
    createFabrikConstraintDialog(chainIdx, chainBoneIdx, boneName, callback = null) {
        ...
        inspector.on_refresh = (o) => {
            ...
            inspector.addButton(null, "Add", { callback: v => {
                ...
                this.editor.FABRIKSolver.setConstraintToBone( this.editor.fabrikChains[chainIdx].name, chainBoneIdx, newConstraint);
                dialog.close();
                if(callback)
                    callback();
            }});
